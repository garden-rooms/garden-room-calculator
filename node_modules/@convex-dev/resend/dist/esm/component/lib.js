...
import { vOptions, vStatus } from "./shared";
...
async function scheduleBatchRun(ctx, options) {
    ...
    const runId = await ctx.scheduler.runAfter(BASE_BATCH_DELAY, internal.lib.makeBatch, { reloop: false });
...
export const makeBatch = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const emails = await ctx.db
            .query("emails")
            .withIndex("by_status_segment", (q) => q.eq("status", "waiting").lte("segment", scanSegment))
            .take(BATCH_SIZE);
...
export const markEmailsSent = internalMutation({
    ...
    returns: v.null(),
    handler: async (ctx, args) => {
        await Promise.all(args.emailIds.map((emailId, i) => ctx.db.patch(emailId, {
            status: "sent",
...
export const handleEmailEvent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const email = await ctx.db
            ...
            .unique();
        if (!email)
            throw new Error("Email not found for resendId: " + resendId);
        const cleanedEvent = {
            type: event.type,
            data: {
                email_id: resendId,
...
export const cleanupOldEmails = internalMutation({
    ...
    handler: async (ctx) => {
        const oldAndDone = await ctx.db
            .query("emails")
            .withIndex("by_finalizedAt", (q) => q.lt("finalizedAt", Date.now() - FINALIZED_EMAIL_RETENTION_MS))
            .take(500);
...
export const cleanupAbandonedEmails = internalMutation({
    ...
    handler: async (ctx) => {
        const oldAndAbandoned = await ctx.db
            .query("emails")
            .withIndex("by_creation_time", (q) => q.lt("_creationTime", Date.now() - ABANDONED_EMAIL_RETENTION_MS))
            .take(500);
